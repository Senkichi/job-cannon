...
def upsert_job(
    ...
    *,
    ...
    Accepts only ParsedJob or UnresolvedParsedJob — the legacy Job->ParsedJob shim
    was removed in Phase 48.07.  Callers must construct a ParsedJob explicitly via
    ``ParsedJob.from_job(job, source_meta=...)`` before calling this function.
    ...
    if not isinstance(parsed, (_ParsedJob, _UnresolvedParsedJob)):
        raise TypeError(
            ...
            "Construct a ParsedJob via ParsedJob.from_job(job, source_meta=...) before calling upsert_job."
        )

    # Score fields are not carried by ParsedJob (they are scorer-pipeline concerns).
    # At ingest time, score=0.0 is the correct default; the AI scoring pipeline
    # updates these fields separately after enrichment.
    _score: float = 0.0
    _score_breakdown: dict = {}

    # Resolve structured locations:
    # - ParsedJob path: parsed.locations_structured was set by the caller (from_job
    #   or a Layer-1 scanner that built JobLocation objects directly).
    ...
    # derive from the location string (callers that omit it, e.g. Layer-2 scanners).
