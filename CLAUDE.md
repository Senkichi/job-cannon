...
- Cascade order (configurable in `config.yaml > providers.fallback_chain`): Ollama → Gemini → Claude Code CLI → Anthropic (production default; see Recipe 1 in config.example.yaml). Each provider can be disabled or rate-limited independently.
