...
today_jobs = conn.execute(
    "SELECT COUNT(*) FROM jobs WHERE date(first_seen) = date('now', 'localtime')"
).fetchone()[0]
...
# Unscored from today
unscored_today = conn.execute(
    "SELECT COUNT(*) FROM jobs WHERE haiku_score IS NULL AND date(first_seen) = date('now', 'localtime')"
).fetchone()[0]
print(f"Unscored from today: {unscored_today}")

# No jd_full at all
no_jd = conn.execute(
    "SELECT COUNT(*) FROM jobs WHERE jd_full IS NULL OR TRIM(jd_full) = ''"
).fetchone()[0]
print(f"No jd_full: {no_jd}")

# No jd_full from today
no_jd_today = conn.execute(
    "SELECT COUNT(*) FROM jobs WHERE (jd_full IS NULL OR TRIM(jd_full) = '') AND date(first_seen) = date('now', 'localtime')"
).fetchone()[0]
print(f"No jd_full from today: {no_jd_today}")

# Stub jd_full (< 200 chars)
stub_jd = conn.execute(
    "SELECT COUNT(*) FROM jobs WHERE jd_full IS NOT NULL AND LENGTH(TRIM(jd_full)) > 0 AND LENGTH(TRIM(jd_full)) < 200"
).fetchone()[0]
...
for row in conn.execute(
    "SELECT enrichment_tier, COUNT(*) as cnt FROM jobs GROUP BY enrichment_tier ORDER BY cnt DESC"
):
    print(f"  {row[0]}: {row[1]}")
...
for row in conn.execute(
    "SELECT enrichment_tier, COUNT(*) as cnt FROM jobs WHERE date(first_seen) = date('now', 'localtime') GROUP BY enrichment_tier ORDER BY cnt DESC"
):
    print(f"  {row[0]}: {row[1]}")
...
missing_sonnet = conn.execute(
    "SELECT COUNT(*) FROM jobs WHERE haiku_score >= 65 AND sonnet_score IS NULL AND jd_full IS NOT NULL AND LENGTH(TRIM(jd_full)) >= 200"
).fetchone()[0]
...
for row in conn.execute("""SELECT dedup_key, title, company, enrichment_tier,
    ...
    LIMIT 10"""):
    print(
        f"  [{row['enrichment_tier']}] [{row['jd_status']}] {row['title'][:40]} @ {row['company'][:25]} | desc_len={row['desc_len']} jd_len={row['jd_len']} | sources={row['sources']} | status={row['pipeline_status']}"
    )
...
for row in conn.execute("""SELECT dedup_key, title, company, enrichment_tier,
    ...
    LIMIT 10"""):
    print(
        f"  [{row['enrichment_tier']}] [{row['jd_status']}] {row['title'][:40]} @ {row['company'][:25]} | desc_len={row['desc_len']} jd_len={row['jd_len']} | sources={row['sources']} | status={row['pipeline_status']} | seen={row['first_seen']}"
    )

# Check for jobs with description > 200 but no jd_full (eager promotion failure?)
eager_miss = conn.execute(
    "SELECT COUNT(*) FROM jobs WHERE jd_full IS NULL AND description IS NOT NULL AND LENGTH(description) > 200"
).fetchone()[0]
print(f"\nEager promotion misses (desc>200 but no jd_full): {eager_miss}")

# Check for jobs with no description at all
no_desc = conn.execute(
    "SELECT COUNT(*) FROM jobs WHERE description IS NULL OR TRIM(description) = ''"
).fetchone()[0]
...
try:
    for row in conn.execute(
        "SELECT model, COUNT(*) as cnt, SUM(cost_usd) as total_cost FROM scoring_costs GROUP BY model"
    ):
...
try:
    month_cost = conn.execute(
        "SELECT SUM(cost_usd) FROM scoring_costs WHERE strftime('%Y-%m', created_at) = strftime('%Y-%m', 'now')"
    ).fetchone()[0]
