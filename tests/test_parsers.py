...
class TestLinkedInParser:
    def test_parse_basic_alert(self):
        jobs = parse_linkedin_alert(SAMPLE_LINKEDIN_BODY)
        assert len(jobs) == 3
    ...
    def test_parse_job_fields(self):
        ...
        assert jobs[0].company == "Thumbtack"
        assert jobs[0].location == "United States"
        assert jobs[0].source == "linkedin"
        assert jobs[0].source_id == "4364166509"

    def test_clean_url(self):
        jobs = parse_linkedin_alert(SAMPLE_LINKEDIN_BODY)
        assert jobs[0].source_url == "https://www.linkedin.com/jobs/view/4364166509/"

    def test_all_titles_parsed(self):
        jobs = parse_linkedin_alert(SAMPLE_LINKEDIN_BODY)
        titles = [j.title for j in jobs]
        assert "Senior Data Scientist" in titles
        assert "Data Scientist, Experimentation" in titles
        assert "Staff Data Scientist" in titles

    def test_with_email_date(self):
        date = datetime(2026, 3, 9)
        jobs = parse_linkedin_alert(SAMPLE_LINKEDIN_BODY, email_date=date)
        assert all(j.posted_date == date for j in jobs)

    def test_empty_body(self):
        assert parse_linkedin_alert("") == []

    def test_no_jobs_body(self):
