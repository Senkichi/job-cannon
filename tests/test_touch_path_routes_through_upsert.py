...
def test_new_source_only_is_touched(conn: sqlite3.Connection):
    r1 = upsert_job(conn, _to_parsed(_make_job()))
    ...
    r2 = upsert_job(conn, _to_parsed(_make_job(source="greenhouse", url="https://ex.co/2")))
...
def test_identical_reingest_is_unchanged(conn: sqlite3.Connection):
    r1 = upsert_job(conn, _to_parsed(_make_job()))
    assert r1.kind == "inserted"
    r2 = upsert_job(conn, _to_parsed(_make_job()))
...
def test_new_salary_is_updated(conn: sqlite3.Connection):
    r1 = upsert_job(conn, _to_parsed(_make_job()))
    assert r1.kind == "inserted"
    r2 = upsert_job(conn, _to_parsed(_make_job(salary_min=180_000)))
...
def test_approved_state_survives_touch(conn: sqlite3.Connection):
    ...
    conn.commit()
    r2 = upsert_job(conn, _to_parsed(_make_job(source="ashby", url="https://ex.co/3")))
    assert r2.kind == "touched"
