...
def _make_parsed(**kw) -> ParsedJob:
    return ParsedJob.from_job(_make_job(**kw))
...
def test_new_source_only_is_touched(conn: sqlite3.Connection):
    r1 = upsert_job(conn, _make_parsed())
    ...
    r2 = upsert_job(conn, _make_parsed(source="greenhouse", url="https://ex.co/2"))
...
def test_identical_reingest_is_unchanged(conn: sqlite3.Connection):
    r1 = upsert_job(conn, _make_parsed())
    assert r1.kind == "inserted"
    r2 = upsert_job(conn, _make_parsed())
...
def test_new_salary_is_updated(conn: sqlite3.Connection):
    r1 = upsert_job(conn, _make_parsed())
    assert r1.kind == "inserted"
    r2 = upsert_job(conn, _make_parsed(salary_min=180_000))
...
def test_approved_state_survives_touch(conn: sqlite3.Connection):
    ...
    conn.commit()
    r2 = upsert_job(conn, _make_parsed(source="ashby", url="https://ex.co/3"))
    assert r2.kind == "touched"
