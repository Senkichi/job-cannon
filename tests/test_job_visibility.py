...
def _insert_job(conn: sqlite3.Connection, dedup_key: str, status: str = "discovered",
                first_seen: str = "2026-04-01T10:00:00") -> None:
    conn.execute(
        """INSERT INTO jobs
            (dedup_key, title, company, location, sources, source_urls,
             first_seen, last_seen, score, score_breakdown, pipeline_status)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
        (dedup_key, "Data Scientist", "ACME Corp", "Remote",
         '["test"]', '["https://example.com"]',
         first_seen, first_seen, 75.0, '{}', status),
    )
    conn.commit()
...
class TestAutoDismiss:
    def test_auto_dismiss_excluded_job_sets_dismissed_status(self, tmp_db_path):
        ...
        conn.execute(
            """INSERT INTO jobs
                (dedup_key, title, company, location, sources, source_urls,
                 first_seen, last_seen, score, score_breakdown, pipeline_status)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
            ("intern|job|remote", "Data Science Intern", "ACME",
             "Remote", '["test"]', '["https://example.com"]',
             "2026-04-01", "2026-04-01", 0, "{}", "discovered"),
        )
        conn.commit()
        conn.close()
        ...
        config = {
            "db": {"path": tmp_db_path},
            "scoring": {"haiku_threshold": 55, "min_score_threshold": 40, "monthly_budget_usd": 25.0},
            "profile": {
    ...
    def test_auto_dismiss_does_not_override_reviewing(self, tmp_db_path):
        ...
        conn.execute(
            ...
            ("intern|job|remote", "Data Science Intern", "ACME",
             "Remote", '["test"]', '["https://example.com"]',
             "2026-04-01", "2026-04-01", 0, "{}", "reviewing"),
        )
        conn.commit()
        conn.close()
