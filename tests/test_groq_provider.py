"""Unit tests for job_finder.web.providers.groq_provider.

Tests cover:
- Subclass relationship (GroqProvider is a BaseProvider)
...
def _make_provider(config: dict | None = None) -> GroqProvider:
    if config is None:
        config = {}
...
def test_groq_provider_is_base_provider_subclass():
...
def test_init_reads_api_key_from_env():
    with patch.dict("os.environ", {"GROQ_API_KEY": "my-key"}):
...
def test_init_custom_api_key_env():
    with patch.dict("os.environ", {"CUSTOM_KEY": "custom-val"}):
        provider = GroqProvider(config={
            "providers": {"groq": {"api_key_env": "CUSTOM_KEY"}}
        })
...
def test_init_strips_trailing_slash():
    with patch.dict("os.environ", {"GROQ_API_KEY": "key"}):
        provider = GroqProvider(config={
            "providers": {"groq": {"base_url": "https://example.com/v1/"}}
        })
...
def test_successful_call_returns_model_result():
    ...
    with patch("job_finder.web.providers.groq_provider.requests.post", return_value=resp):
...
def test_schema_embedded_in_system_prompt():
    ...
    with patch("job_finder.web.providers.groq_provider.requests.post", return_value=resp) as mock_post:
        provider.call(
            model="llama-3.1-8b-instant",
            system="Score jobs",
...
def test_rate_limit_429_raises_http_error():
    """429 from Groq must raise HTTPError so the cascade catches it and falls through."""
    provider = _make_provider()
