...
Named test_m084_* to match the issue number (#170); the migration itself
is version 85 because version 84 was taken by the parser auto-heal Phase A
migration (corpus_sample + source_health tables).
"""
...
def test_m085_adds_direct_url_columns(tmp_path):
...
def test_m085_confidence_check_constraint(tmp_path):
    conn = _migrated_db(tmp_path)
    conn.execute(
