...
test runs against the real, fully-migrated schema (m104 columns present).
"""
...
class TestPairAtomicReconciler:
    def test_higher_rank_incoming_replaces_whole_tuple(self):
        cols, changed = _reconcile_salary_pair_for_upsert(
            150_000, 200_000, "annual", "USD", "ats_structured",
            3_000, 251_000, "unknown", "USD", "feed_string",
        )
    ...
    def test_equal_rank_incoming_refreshes(self):
        ...
        cols, changed = _reconcile_salary_pair_for_upsert(
            160_000, 210_000, "annual", "USD", "ats_structured",
            150_000, 200_000, "annual", "USD", "ats_structured",
        )
        assert changed is True
    ...
    def test_null_stored_pair_fills_regardless_of_rank(self):
        cols, changed = _reconcile_salary_pair_for_upsert(
            120_000, 150_000, "annual", "USD", "feed_string",
    ...
    def test_null_incoming_never_clobbers(self):
        cols, changed = _reconcile_salary_pair_for_upsert(
            None, None, "unknown", "USD", "feed_string",
            150_000, 200_000, "annual", "USD", "ats_structured",
        )
        assert changed is False
        assert cols == {}
    ...
    def test_legacy_null_provenance_overwritable_by_anything(self):
        ...
        cols, changed = _reconcile_salary_pair_for_upsert(
            120_000, 150_000, "annual", "USD", "feed_string",
            9_000, 338_000, "unknown", "USD", None,
        )
        assert changed is True
        assert cols["salary_min"] == 120_000
...
class TestObservationLog:
    ...
    def test_caps_oldest_dropped(self):
        ...
        incoming = [{"provenance": "ats_structured", "raw_text": "new", "min_value": 9, "max_value": 9}]
...
class TestEnrichmentRankGate:
    def test_llm_extract_cannot_overwrite_ats_structured(self):
        cols, dropped = _reconcile_salary_for_write(
            100_000, 120_000, 150_000, 200_000,
...
class TestUpsertConvergence:
    def test_jooble_then_greenhouse_converges_to_greenhouse(self, migrated_db):
        ...
        upsert_job(
            conn,
            _make_parsed(
                title=title, smin=3_000, smax=251_000, provenance="feed_string",
                period="unknown", raw_text="$3K - $251K",
            ),
        )
        ...
        upsert_job(
            conn,
            _make_parsed(
                title=title, smin=170_000, smax=200_000, provenance="ats_structured",
                period="annual", raw_text="gh",
            ),
        )
        after = _read(conn, dk)
    ...
    def test_touch_path_no_duplicate_observation(self, migrated_db):
        ...
        p = _make_parsed(
            title=title, smin=120_000, smax=150_000, provenance="feed_string",
            period="annual", raw_text="$120K - $150K",
        )
        upsert_job(conn, p)
        dk = f"reconco|{_norm(title)}"
    ...
    def test_greenhouse_rescan_refreshes_equal_rank(self, migrated_db):
        ...
        upsert_job(
            conn,
            _make_parsed(
                title=title, smin=150_000, smax=200_000, provenance="ats_structured",
                period="annual", raw_text="gh-v1",
            ),
        )
        ...
        upsert_job(
            conn,
            _make_parsed(
                title=title, smin=160_000, smax=210_000, provenance="ats_structured",
                period="annual", raw_text="gh-v2",
            ),
        )
        dk = f"reconco|{_norm(title)}"
        after = _read(conn, dk)
...
class TestParsedJobPlumbing:
    def test_source_meta_observation_seeds_log(self, migrated_db):
        ...
        title = "Data Engineer"
        p = _make_parsed(
            title=title, smin=140_000, smax=180_000, provenance="ats_structured",
            period="annual", raw_text="gh-de",
        )
    ...
    def test_no_salary_meta_leaves_provenance_null(self, migrated_db):
        ...
        job = Job(
            title=title, company="ReconCo", location="Remote", source="portal_jooble",
            source_url="https://example.com/pm", description="y" * 300,
        )
...
# ---------------------------------------------------------------------------
# m104 heal — salvage + quarantine on seeded corrupt rows
# ---------------------------------------------------------------------------
...
class TestM104Heal:
    ...
    def test_greenhouse_cents_salvaged(self, migrated_db):
        from job_finder.web.migrations.m104_salary_provenance import _heal
    ...
    def test_subfloor_min_quarantined(self, migrated_db):
        from job_finder.web.migrations.m104_salary_provenance import (
